...
//debug: uncomment 120 176 428 293 217 186 147
typedef struct tcb
{
	enum {
        CREATED,
...
void mythread_mutex_lock(mythread_mutex_t *m)
{
	while(1)
	{
		if (m->id!=-1)
		{
			// printf("186--Wiating for lock to be freed. Thread t %d\n", currentThread->id);
			myThread_yield();
		}
		else
		{
...
void alarmHandlr (int a)
{
	...
	if (a==SIGALRM)
	{
		...
		else
		{
			...
			{
				// printf("221--Signal recieved in execution of thread %d. Saving context.\n", currentThread->id );
...
void myThread_yield(void)
{
	// printf("293--Inside yield function\n");
	if (setjmp(currentThread->currentenv)==0)
	{
		currentThread->status=READY;
    	myThread_schedule();
